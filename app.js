...
function pages() {
  return [
    ...
    scene(`<div class="kicker">chapter 10 · before you go</div><h2>I have one tiny<br><em>birthday wish.</em></h2><p class="lede">${esc(CONFIG.giftPrompt)}</p>${state.submitted ? `<div class="final-note"><span>✦</span><h3>Your wish is waiting for me.</h3><p>Thank you for being you, ${esc(state.recipient)}.</p><p class="reply-time">Saved on ${esc(state.reply.sentAt)}${state.reply.rating ? ` · Rated ${esc(state.reply.rating)}/10` : ""}.</p></div>` : `<form id="gift-form" class="gift-form"><label for="gift-input">Your birthday wish</label><textarea id="gift-input" placeholder="Type your wish here..." aria-label="Your birthday wish" required></textarea><label for="gift-feedback">A note about this site</label><textarea id="gift-feedback" placeholder="What did you enjoy or what could feel better?" aria-label="Feedback about the site" required></textarea><label for="gift-name">Your name</label><input id="gift-name" type="text" placeholder="Your name" aria-label="Your name" required><label for="gift-rating">Rate the site out of 10</label><div class="gift-rating-row"><div class="rating-field"><input id="gift-rating" type="number" min="1" max="10" step="1" placeholder="8" aria-label="Rate the site out of 10" required><span>/ 10</span></div></div><button class="button" type="submit">Send my reply <span>↗</span></button></form><p class="hint">Your wish, feedback, name, and rating will be visible in the sender view.</p>`}${CONFIG.catGif ? `<div class="final-cat-wrap"><img class="final-cat-gif" src="${esc(CONFIG.catGif)}" alt="cat gif" loading="lazy"></div>` : ""}<p class="signature">with love,<br><strong>${esc(CONFIG.sender)}</strong></p>`, "final-page")
  ];
}
...
document.addEventListener("submit", (event) => {
  if (event.target.id === "gift-form") {
    ...
    state.reply = { name: name || state.recipient, message, feedback, rating: ratingValue, sentAt: new Date().toLocaleString() };
