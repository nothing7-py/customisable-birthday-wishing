...
const go = (page) => { state.page = Math.max(0, Math.min(11, page)); render(); window.scrollTo({ top: 0, behavior: "smooth" }); };
const photo = (item, index) => item.src ? `<img src="${esc(item.src)}" alt="${esc(item.caption)}" loading="lazy">` : `<div class="photo-placeholder"><span>${["01", "02", "03"][index]}</span><small>add a photo<br>in config.js</small></div>`;
